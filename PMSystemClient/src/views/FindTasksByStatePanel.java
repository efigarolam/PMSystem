 ...
 */
public class FindTasksByStatePanel extends AbstractPanelView {
    ...
    public FindTasksByStatePanel(String args[]) {
    ...
    public void configure() {
        ...
        jMenuItem6.setText(__("Update Progress"));
    ...
    public void refresh() {
        ...
        if(!state.toString().equals("")) {
            TaskData[] result = this.tasksController.getTasks("State = '"+state.toString()+"'");
            if(result[0].id == 0) {
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Started", "Paused", "Stoped", "Finished", "Inactive" }));
    ...
    private void jMenuItem5ActionPerformed(ActionEvent evt) {
        int selectedRowIndex = jTable1.getSelectedRow();
        if(selectedRowIndex >= 0) {
            if(this.alertsHelper.showConfirm(__("Do you want to restore the task?"), "question")) {
                Object selectedRow = jTable1.getValueAt(selectedRowIndex, 0);
                int id_task = Integer.parseInt(selectedRow.toString());
                if(this.tasksController.restoreTask(id_task)) {
                    this.alertsHelper.showAlert(__("Task restored correctly!"), "information");
                    this.refresh();
                } else {
                    this.alertsHelper.showAlert(__("Ups! an unexpected error has ocurred"), "error");
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if(this.btnTrashBhvr) {
            ...
            if(jTable1.getSelectedRowCount() > 0) {
                int[] selectedRowsIndex = jTable1.getSelectedRows();
                int errors = 0;
                if(this.alertsHelper.showConfirm(__("Do you want to restore the task(s)?"), "question")) {
                    for(int i = 0; i < selectedRowsIndex.length; i++) {
                        Object selectedRow = jTable1.getValueAt(selectedRowsIndex[i], 0);
                        int id_user = Integer.parseInt(selectedRow.toString());
                        if(!this.tasksController.restoreTask(id_user)) {
                            errors++;
                        }                   
                    }
                    ...
                    if(selectedRowsIndex.length == errors) {
                        ...
                        this.alertsHelper.showAlert(__("Task(s) restoredcorrectly!"), "information");
                    } else if(selectedRowsIndex.length > errors) {
                        this.alertsHelper.showAlert(__("Some task(s) restored correctly, but others failed!"), "information");
                    } 
