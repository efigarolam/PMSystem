 ...
 */
public class FindUsersByEmailPanel extends AbstractPanelView {
    ...
    public FindUsersByEmailPanel(String args[]) {
    ...
    public void refresh() {
        String email = this.stringsHelper.clean(jTextField1.getText());
        
        if(!email.equals("")) {
            UserData[] result = this.usersController.getUsers("Email LIKE '%"+email+"%' AND State = 'Active'");
            if(result[0].id == 0) {
                this.jPanel1.setVisible(false);
                this.alertsHelper.showAlert(__("There are no users with that email"), "information");
            } else {
