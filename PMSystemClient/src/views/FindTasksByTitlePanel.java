 ...
 */
public class FindTasksByTitlePanel extends AbstractPanelView {
    ...
    public FindTasksByTitlePanel(String args[]) {
    ...
    public void refresh() {
        String title = this.stringsHelper.clean(jTextField1.getText());
        
        if(!title.equals("")) {
            TaskData[] result = this.tasksController.getTasks("Title LIKE '%"+title+"%' AND State != 'Inactive'");
            if(result[0].id == 0) {
                this.jPanel1.setVisible(false);
                this.alertsHelper.showAlert(__("There are no tasks with that name"), "information");
            } else {
            ...
            this.alertsHelper.showAlert(__("Please write a title"), "error");
        }
    }
