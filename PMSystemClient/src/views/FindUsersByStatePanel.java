 ...
 */
public class FindUsersByStatePanel extends AbstractPanelView {
    
    private UsersController usersController;
    private DefaultTableModel tModel;
    private boolean btnSelectBhvr = true;
    private boolean btnTrahsBhvr  = true;
    
    /** Creates new form FindUsersByUsername */
    public FindUsersByStatePanel(String args[]) {
    ...
    public void configure() {
        ...
        jMenuItem6.setText(__("Send Message"));
    ...
    public void refresh() {
        ...
        if(!state.toString().equals("")) {
            UserData[] result = this.usersController.getUsers("State = '"+state.toString()+"'");
            if(result[0].id == 0) {
                this.jPanel1.setVisible(false);
                this.alertsHelper.showAlert(__("There are no users with that state"), "information");
            } else {
                if(state.equals("Inactive")) {
                    this.btnTrahsBhvr = false;
            ...
            this.alertsHelper.showAlert(__("Please write a state"), "error");
        }
    }
    ...
    private void jMenuItem5ActionPerformed(ActionEvent evt) {
        int selectedRowIndex = jTable1.getSelectedRow();
        if(selectedRowIndex >= 0) {
            if(this.alertsHelper.showConfirm(__("Do you want to restore the user?"), "question")) {
                Object selectedRow = jTable1.getValueAt(selectedRowIndex, 0);
                int id_user = Integer.parseInt(selectedRow.toString());
                if(this.usersController.restoreUser(id_user)) {
                    this.alertsHelper.showAlert(__("User restored correctly!"), "information");
                    this.refresh();
                } else {
                    this.alertsHelper.showAlert(__("Ups! an unexpected error has ocurred"), "error");
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if(this.btnTrahsBhvr) {
            ...
            int[] selectedRowsIndex = jTable1.getSelectedRows();
            int errors = 0;
                if(this.alertsHelper.showConfirm(__("Do you want to restore the user(s)?"), "question")) {
                    for(int i = 0; i < selectedRowsIndex.length; i++) {
                        Object selectedRow = jTable1.getValueAt(selectedRowsIndex[i], 0);
                        int id_user = Integer.parseInt(selectedRow.toString());
                        if(!this.usersController.restoreUser(id_user)) {
                    ...
                    if(selectedRowsIndex.length == errors) {
                        ...
                        this.alertsHelper.showAlert(__("User(s) restored correctly!"), "information");
                    } else if(selectedRowsIndex.length > errors) {
                        this.alertsHelper.showAlert(__("Some user(s) restored correctly, but others failed!"), "information");
                    } 
