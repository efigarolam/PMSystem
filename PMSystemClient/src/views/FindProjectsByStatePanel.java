 ...
 */
public class FindProjectsByStatePanel extends AbstractPanelView {
    
    private ProjectsController projectsController;
    private DefaultTableModel tModel;
    private boolean btnSelectBhvr = true;
    private boolean btnTrashBhvr = true;
    
    /** Creates new form FindUsersByUsername */
    public FindProjectsByStatePanel(String args[]) {
    ...
    public void configure() {
        ...
        jMenuItem6.setText(__("Send message to leader"));
        ...
        jMenuItem7.setText(__("View Gantt chart"));
    ...
    public void refresh() {
        Object state = jComboBox1.getSelectedItem();
        if(!state.toString().equals("")) {
            ProjectData[] result = this.projectsController.getProjects("State = '"+state.toString()+"'");
            if(result[0].id == 0) {
                this.jPanel1.setVisible(false);
                this.alertsHelper.showAlert(__("There are no projects with that state"), "information");
            } else {
                if(state.equals("Inactive")) {
                    jButton3.setText(__("Restore"));
                    this.btnTrashBhvr = false;
                    jMenuItem5 = new JMenuItem();
                    jMenuItem5.setText(__("Restore"));
                    ...
                    jPopupMenu1.add(jMenuItem5, 2);
                } else {
                    jButton3.setText(__("Send to trash"));
                    this.btnTrashBhvr = true;
                    jPopupMenu1.remove(2);
                    jPopupMenu1.add(jMenuItem3, 2);
                }
                ...
                for(int i = 0; i < result.length; i++) {
                    ...
                    row[1] = Integer.toString(result[i].id_user);
                    row[2] = result[i].name;
                    row[3] = result[i].user_name;
                    row[4] = result[i].description;
            ...
            this.alertsHelper.showAlert(__("Please select a state"), "error");
        } 
    }
    ...
    private void jMenuItem5ActionPerformed(ActionEvent evt) {
        int selectedRowIndex = jTable1.getSelectedRow();
        if(selectedRowIndex >= 0) {
            if(this.alertsHelper.showConfirm(__("Do you want to restore the project?"), "question")) {
                Object selectedRow = jTable1.getValueAt(selectedRowIndex, 0);
                int id_project = Integer.parseInt(selectedRow.toString());
                if(this.projectsController.restoreProject(id_project)) {
                    this.alertsHelper.showAlert(__("Project restored correctly!"), "information");
                    this.refresh();
                } else {
                    this.alertsHelper.showAlert(__("Ups! an unexpected error has ocurred"), "error");
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if(this.btnTrashBhvr) {
            if(jTable1.getSelectedRowCount() > 0) {
                int[] selectedRowsIndex = jTable1.getSelectedRows();
                int errors = 0;
                if(this.alertsHelper.showConfirm(__("Do you want to send to trash the project(s)?"), "question")) {
                    for(int i = 0; i < selectedRowsIndex.length; i++) {
                        Object selectedRow = jTable1.getValueAt(selectedRowsIndex[i], 0);
                        int id_user = Integer.parseInt(selectedRow.toString());
                        if(!this.projectsController.trashProject(id_user)) {
                            errors++;
                        }                   
                    }
                    ...
                    if(selectedRowsIndex.length == errors) {
                        ...
                        this.alertsHelper.showAlert(__("Project(s) sent to trash correctly!"), "information");
                    } else if(selectedRowsIndex.length > errors) {
                        this.alertsHelper.showAlert(__("Some project(s) sent to trash correctly, but others failed!"), "information");
                    } 
            ...
            if(jTable1.getSelectedRowCount() > 0) {
                int[] selectedRowsIndex = jTable1.getSelectedRows();
                int errors = 0;
                if(this.alertsHelper.showConfirm(__("Do you want to restore the project(s)?"), "question")) {
                    for(int i = 0; i < selectedRowsIndex.length; i++) {
                        Object selectedRow = jTable1.getValueAt(selectedRowsIndex[i], 0);
                        int id_user = Integer.parseInt(selectedRow.toString());
                        if(!this.projectsController.restoreProject(id_user)) {
                            errors++;
                        }                   
                    }
                    ...
                    if(selectedRowsIndex.length == errors) {
                        ...
                        this.alertsHelper.showAlert(__("Project(s) restored correctly!"), "information");
                    } else if(selectedRowsIndex.length > errors) {
                        this.alertsHelper.showAlert(__("Some project(s) restored correctly, but others failed!"), "information");
                    }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        if(jTable1.getSelectedRowCount() > 0) {
            int[] selectedRowsIndex = jTable1.getSelectedRows();
            int errors = 0;
            if(this.alertsHelper.showConfirm(__("Do you want to delete the project(s)?"), "question")) {
                for(int i = 0; i < selectedRowsIndex.length; i++) {
                    Object selectedRow = jTable1.getValueAt(selectedRowsIndex[i], 0);
                    int id_user = Integer.parseInt(selectedRow.toString());
                    if(!this.projectsController.deleteProject(id_user)) {
                        errors++;
                    }                   
                }
                ...
                if(selectedRowsIndex.length == errors) {
                    ...
                    this.alertsHelper.showAlert(__("Project(s) deleted correctly!"), "information");
                } else if(selectedRowsIndex.length > errors) {
                    this.alertsHelper.showAlert(__("Some project(s) deleted correctly, but others failed!"), "information");
                } 
